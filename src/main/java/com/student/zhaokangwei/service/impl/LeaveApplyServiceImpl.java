...
import com.student.zhaokangwei.mapper.ILeaveApplyMapper;
...
public class LeaveApplyServiceImpl extends ServiceImpl<ILeaveApplyMapper, LeaveApply> implements ILeaveApplyService {
    ...
    @Override
    public IPage<Map<String, Object>> pageMyLeaveApplyInfo(Integer pageIndex, Integer pageSize) {
        ...
        IPage<Map<String, Object>> mapIPage = leaveApplyMapper.pageLeaveApplyJoinUser(page, queryWrapper);
        for (Map<String, Object> map : mapIPage.getRecords()) {
            if (map.get("process_instance_id") == null) continue;
            //根据从请假记录里面获取的实例ID，来获取对应的任务集合
...
//                taskObject.put("deposeUser", userService.getById(Integer.parseInt(task.getAssignee())).getRealname());
    ...
    public void submitApply(Byte typeId, LocalDateTime starttime, LocalDateTime endtime, String reason) {
        ...
        //获取“请假流程”的信息
        Flow flow = flowService.getById(1);
    ...
    public Map<String, Object> approval(String processInstanceId) {
        ...
        Map<String, Object> map = leaveApplyMapper.getLeaveApplyJoinUser(queryWrapper);
        if (map == null) {
            throw new ServiceValidationException("未找到对应的请假记录", 402);
        }
        return map;
    }

    @Override
    public void passDispose(int leaveApplyId, String disposeReason, String taskID) {
        ...
        if (count == 0) {
            ...
            changeState(leaveApplyId, disposeReason, (byte) 3);
    ...
    @Override
    public void rejectDispose(int leaveApplyId, String disposeReason, String taskID) {
        //1. 终止当前任务所在的实例流程
        Task task = taskService.createTaskQuery().taskId(taskID).singleResult();
        ...
        //2. 更新请假记录的state值，改为2
        changeState(leaveApplyId, disposeReason, (byte) 2);
     ...
     * @param state
     */
    private void changeState(int leaveApplyId, String disposeReason, byte state) {
        //直接从Security上下文对象中取出当前请求者的身份信息
        Integer userId = Integer.parseInt(SecurityContextHolder.getContext().getAuthentication().getPrincipal().toString());
        User user = userService.getById(userId);

        QueryWrapper<LeaveApply> leaveApplyQueryWrapper = new QueryWrapper<>();
        leaveApplyQueryWrapper.eq("id", leaveApplyId);
        LeaveApply leaveApply = new LeaveApply();
        leaveApply.setStateId(state); //设置新的状态值
        leaveApply.setDisposeReason(user.getRealname() + ":" + disposeReason); //更新处理意见
        update(leaveApply, leaveApplyQueryWrapper); //执行UPDATE的SQL语句
    }
